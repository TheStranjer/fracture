...
RSpec.describe Fracture::Tools::RemoveRole do
  ...
  describe '.tool_name' do
    it 'is remove_role' do
      expect(described_class.name_value).to eq('remove_role')
    end
  end
  ...
  describe '.call' do
    ...
    before do
      ...
      allow(member).to receive(:remove_role)
    end

    it 'returns an MCP tool response' do
      response = described_class.call(guild_id: '111222333', member_id: '1001', role_id: '5001')
      expect(response).to be_a(MCP::Tool::Response)
    end

    it 'removes the role from the member' do
      expect(member).to receive(:remove_role).with(role)
      described_class.call(guild_id: '111222333', member_id: '1001', role_id: '5001')
    end

    it 'returns a success response' do
      parsed = parse_response(described_class.call(guild_id: '111222333', member_id: '1001', role_id: '5001'))
    ...
    context 'when the guild is not found' do
      ...
      it 'returns an error response' do
        parsed = parse_response(described_class.call(guild_id: '999', member_id: '1001', role_id: '5001'))
    ...
    context 'when the member is not found' do
      ...
      it 'returns an error response' do
        parsed = parse_response(described_class.call(guild_id: '111222333', member_id: '9999', role_id: '5001'))
    ...
    context 'when the role is not found' do
      it 'returns an error response' do
        parsed = parse_response(described_class.call(guild_id: '111222333', member_id: '1001', role_id: '9999'))
