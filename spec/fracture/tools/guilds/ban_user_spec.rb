...
RSpec.describe Fracture::Tools::BanUser do
  ...
  describe '.tool_name' do
    it 'is ban_user' do
      expect(described_class.name_value).to eq('ban_user')
    end
  end
  ...
  describe '.call' do
    ...
    it 'returns an MCP tool response' do
      response = described_class.call(guild_id: '111222333', user_id: '1001', reason: 'spamming')
      expect(response).to be_a(MCP::Tool::Response)
    end

    it 'bans the user with the given reason' do
      expect(server).to receive(:ban).with(1001, reason: 'spamming')
      described_class.call(guild_id: '111222333', user_id: '1001', reason: 'spamming')
    end

    it 'returns a success response' do
      parsed = parse_response(described_class.call(guild_id: '111222333', user_id: '1001', reason: 'spamming'))
    ...
    context 'when the guild is not found' do
      ...
      it 'returns an error response' do
        parsed = parse_response(described_class.call(guild_id: '999', user_id: '1001', reason: 'spamming'))
