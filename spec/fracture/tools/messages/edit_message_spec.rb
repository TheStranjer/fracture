...
RSpec.describe Fracture::Tools::EditMessage do
  ...
  describe '.tool_name' do
    it 'is edit_message' do
      expect(described_class.name_value).to eq('edit_message')
    end
  end
  ...
  describe '.call' do
    ...
    before do
      ...
      allow(message).to receive(:edit)
    end

    context 'when editing in a guild channel' do
      before do
      ...
      it 'returns an MCP tool response' do
        response = described_class.call(
          guild_id: '111222333', channel_id: '100200300', message_id: '999888777', content: 'updated'
        )
        expect(response).to be_a(MCP::Tool::Response)
      end

      it 'edits the message with the new content' do
        expect(message).to receive(:edit).with('updated')
    ...
    context 'when editing in a DM' do
      before do
        allow(discord_client).to receive(:pm_channel).with(1001).and_return(channel)
      end

      it 'resolves the DM channel and edits the message' do
        expect(message).to receive(:edit).with('updated')
        described_class.call(user_id: '1001', message_id: '999888777', content: 'updated')
      end
    ...
    context 'when the message is not found' do
      ...
      it 'returns an error response' do
        parsed = parse_response(
          described_class.call(
            guild_id: '111222333', channel_id: '100200300', message_id: '404', content: 'updated'
          )
        )
    ...
    context 'when neither channel_id nor user_id is provided' do
      it 'returns an error response' do
        parsed = parse_response(described_class.call(message_id: '999888777', content: 'updated'))
